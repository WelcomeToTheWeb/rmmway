...
// ---- fixtures ---------------------------------------------------------------
const DEVICES = [
  { id: "dev-web01", hostname: "web-01", os: "linux", arch: "amd64", agent_version: "0.4.2", interfaces: ["10.0.4.11"], tags: ["web", "prod"], online: true, first_seen: "2026-05-02T09:14:00Z", last_seen: new Date(Date.now() - 4000).toISOString() },
  { id: "dev-db01", hostname: "db-01", os: "linux", arch: "amd64", agent_version: "0.4.2", interfaces: ["10.0.4.21"], tags: ["db", "prod"], online: true, first_seen: "2026-05-02T09:20:00Z", last_seen: new Date(Date.now() - 6000).toISOString() },
  { id: "dev-fs01", hostname: "win-fs-01", os: "windows", arch: "amd64", agent_version: "0.4.2", interfaces: ["10.0.4.31"], tags: ["files"], online: true, first_seen: "2026-05-15T11:02:00Z", last_seen: new Date(Date.now() - 9000).toISOString() },
  { id: "dev-gw03", hostname: "iot-gw-03", os: "linux", arch: "arm64", agent_version: "0.4.1", interfaces: ["10.0.9.3"], tags: ["iot"], online: false, first_seen: "2026-06-01T08:00:00Z", last_seen: new Date(Date.now() - 42 * 60 * 60 * 1000).toISOString() },
];
const SERIES = [
...
function value(name, t, i) {
  ...
  switch (name) {
    case "cpu.utilization_percent":
      return clamp(34 + 22 * Math.sin(day * 2 * Math.PI - 1.2) + jitter * 9, 2, 99);
    case "cpu.load1":
      return Math.max(0.1, (34 + 22 * Math.sin(day * 2 * Math.PI - 1.2)) / 32);
    case "mem.used_percent":
      return clamp(61 + 7 * Math.sin(day * 2 * Math.PI * 2) + jitter * 3, 20, 97);
    case "mem.used_bytes_total":
      return (16 * 1024 ** 3) * (0.55 + 0.1 * Math.sin(day * 2 * Math.PI) + jitter * 0.02);
    case "disk.used_percent":
      return clamp(71.2 + i * 0.004 + jitter * 0.15, 0, 100);
    case "net.rx_bytes_total":
      return 220000 + 180000 * Math.abs(Math.sin(i * 0.35)) + Math.abs(jitter) * 90000;
    case "net.tx_bytes_total":
      return 90000 + 60000 * Math.abs(Math.sin(i * 0.21 + 1)) + Math.abs(jitter) * 40000;
...
const EVENTS = [
  ...
  ["info", "command run_script succeeded", { command_id: "c-9f21", exit_code: 0 }],
...
const COMMANDS = {
  pending: [
    { Id: "c-a7f3", IssuedAtMs: Date.now() - 42000, Action: { RunScript: { Script: "systemctl restart nginx", Lang: "sh" } } },
  ],
  results: [
    { command_id: "c-9f21", status: 3, exit_code: 0, stdout_tail: "nginx restarted\nactive (running)", stderr_tail: "", error: "", completed_at_ms: Date.now() - 35 * 60 * 1000 },
    { command_id: "c-41c8", status: 4, exit_code: 2, stdout_tail: "", stderr_tail: "journalctl: unrecognized option '--since=42d'", error: "", completed_at_ms: Date.now() - 5 * 3600e3 },
  ],
};
...
http
  .createServer(async (req, res) => {
    ...
    try {
      if (p === "/api/setup/status") return json(res, { available: true, setup: true });
      if (p === "/api/login") {
        let body = "";
        for await (const c of req) body += c;
        const u = JSON.parse(body || "{}");
        if (u.username === "admin")
          return json(res, { token: "evidence-token", expiry: "2030-01-01T00:00:00Z", capabilities: [] });
        return json(res, { error: "invalid username or password" }, 401);
      }
      if (p === "/api/devices") return json(res, DEVICES);
      if (p === "/api/alerts/counts") return json(res, { open: 2, acked: 1, resolved: 0 });
      if (p.startsWith("/api/alerts")) return json(res, []);
      if (p === "/healthz") return json(res, { ok: true, version: "0.4.2", probes: { db: "ok" } });
      let m;
      if ((m = /^\/api\/devices\/([^/]+)\/metrics\/series$/.exec(p))) {
        ...
        return json(res, seriesPayload(dev.id, name, src, url.searchParams.get("range") || "24h"));
      }
      if ((m = /^\/api\/devices\/([^/]+)\/metrics$/.exec(p))) {
        ...
        return json(res, {
          ...
          series: list.map((s) => {
            ...
            return { name: s.name, source: s.source, last: pay.last, count: pay.count };
          }),
        });
      }
      if ((m = /^\/api\/devices\/([^/]+)\/commands$/.exec(p)))
        return json(res, m[1] === "dev-web01" ? COMMANDS : { pending: [], results: [] });
      ...
      const body = await readFile(file).catch(async () => readFile(join(DIST, "index.html")));
  ...
  .listen(PORT, () => console.log(`evidence server on :${PORT} serving ${DIST}`));
