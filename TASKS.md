...
**Additional step — bring-your-own reverse proxy (self-hosting):** A self-hoster may run their own reverse proxy (Nginx, Traefik, HAProxy, or their own Caddy) in front of the stack instead of the bundled Caddy edge. The `caddy` service sits behind an opt-in compose profile (`profiles: ["edge"]`); the default `make prod` passes `--profile edge` so Caddy still comes up, while a self-hoster runs `make prod-byoproxy` (base file + `docker-compose.byoproxy.yml`, no `edge` profile) — freeing host ports 80/443 — which publishes the operator API on `${RMMWAY_HTTP_PORT:-8080}:8080` and the SPA on `${RMMWAY_FRONTEND_PORT:-8081}:8080` for the external proxy to forward to. The external proxy then terminates TLS and forwards `/api/*`, `/agent/*`, and `/healthz*` to the API port (SSE buffering off, `X-Forwarded-Proto`/`X-Forwarded-For` preserved) and the SPA to the frontend port (a containerized proxy attached to the shared `rmmway-prod` network can instead proxy `server:8080`/`frontend:8080` directly, with no host ports). The mTLS gRPC agent port (`RMMWAY_AGENT_MTLS_PORT`, default 50052) is published directly by the server either way and is unchanged. Documented in the README "Production stack (A-1)" section (Nginx + Caddy-on-host examples) and `.env.prod.example`.
**Definition of done (BYO proxy):** `make prod-byoproxy` boots the stack with the bundled Caddy off; an external proxy fronting the published API + SPA ports serves the UI + API over TLS (including the live SSE stream), and agents still reach `<domain>:50052` unchanged.
