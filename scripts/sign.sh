...
#   - agent/dist/rmmway-agent-*        (the 6 static binaries)
...
agent/dist/rmmway-agent-windows-arm64.exe"
...
agent/dist/rmmway-agent-windows-arm64.exe.cdx.json"
...
echo "==> SHA256SUMS (root, covers all shipped artifacts)"
{
  (cd agent/dist && sha256sum rmmway-agent-linux-amd64 rmmway-agent-linux-arm64 \
      ...
      rmmway-agent-windows-amd64.exe rmmway-agent-windows-arm64.exe) \
  ...
  (cd agent/dist && sha256sum rmmway-agent-linux-amd64.cdx.json \
      ...
      rmmway-agent-darwin-arm64.cdx.json \
      rmmway-agent-windows-amd64.exe.cdx.json rmmway-agent-windows-arm64.exe.cdx.json) \
    | awk '{print $1"  agent/dist/"$2}'
